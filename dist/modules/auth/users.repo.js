...
export function usersRepo(db) {
    return {
        ...
        async findById(id) {
            ...
            if (error)
                throw Internal(`users.findById: ${error.message}`);
            return toRecord(data);
        },
        async findByEmail(email) {
            const { data, error } = await db.from(TABLE).select('*').eq('email', email.toLowerCase()).maybeSingle();
            if (error)
                throw Internal(`users.findByEmail: ${error.message}`);
            return toRecord(data);
        },
        async findByUsername(username) {
            const { data, error } = await db.from(TABLE).select('*').eq('username', username.toLowerCase()).maybeSingle();
            if (error)
                throw Internal(`users.findByUsername: ${error.message}`);
            return toRecord(data);
        },
        async insert(rec) {
            const { data, error } = await db
                ...
                .single();
            if (error || !data)
                throw Internal(`users.insert: ${error?.message ?? 'no data'}`);
            return toRecord(data);
        },
        async touchLogin(id) {
            const { error } = await db.from(TABLE).update({ last_login_at: new Date().toISOString() }).eq('id', id);
            if (error)
                throw Internal(`users.touchLogin: ${error.message}`);
        },
        async setApiKeyId(id, apiKeyId) {
            const { error } = await db.from(TABLE).update({ api_key_id: apiKeyId }).eq('id', id);
            if (error)
                throw Internal(`users.setApiKeyId: ${error.message}`);
        },
        async list(opts = {}) {
            ...
            if (error)
                throw Internal(`users.list: ${error.message}`);
