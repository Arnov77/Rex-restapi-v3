 ...
 */
async function fetchViaMobileScrape(url, signal) {
    ...
    if (!res.ok)
        throw new Error(`Facebook returned ${res.status}`);
 ...
 */
async function fetchViaDesktopScrape(url, signal) {
    const res = await fetch(url, {
        headers: {
    ...
    if (!res.ok)
        throw new Error(`Facebook desktop returned ${res.status}`);
    const html = await res.text();
    const media = [];
    ...
    try {
        ...
        try {
            ...
            if (json.status === 'error')
                throw new Error(json.text || 'cobalt failed');
            const media = [];
            if (json.status === 'stream' || json.status === 'redirect' || json.status === 'tunnel') {
                if (json.url)
                    media.push({ type: 'video', url: json.url, quality: 'hd' });
            }
