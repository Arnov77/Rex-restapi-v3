...
async function normalizeUrl(url, signal) {
    let resolved = url;
    if (/fb\.watch|facebook\.com\/share\//i.test(url)) {
        try {
            ...
            if (/login|checkpoint/i.test(u.pathname)) {
                ...
                if (next)
                    resolved = decodeURIComponent(next);
            }
        }
        catch {
            resolved = url;
...
export async function downloadFacebook(url, signal) {
    ...
    try {
        ...
        const code = err?.code;
        const noVideo = err?.httpStatus === 400 ||
            (code && /fetch\.(empty|short|fail|critical)|content\.(post|media).*(unavailable|private)|link\.unsupported/i.test(code));
        if (noVideo) {
            throw new AppError(422, 'FACEBOOK_NO_VIDEO', 'No downloadable video found. This downloader supports Facebook videos/reels only — ' +
