...
const Igmp3Query = z.object({
    url: z.string().url().refine((u) => /instagram\.com|instagr\.am/i.test(u), { message: 'Must be a valid Instagram URL' }),
});
 ...
 */
function cobaltError(code, status) {
    switch (code) {
        case 'error.api.fetch.empty':
        case 'error.api.fetch.fail':
        case 'error.api.fetch.critical':
            return new AppError(502, 'AUDIO_EXTRACTION_FAILED', 'Instagram returned no media. The post may be private, removed, or photo-only ' +
        ...
        case 'error.api.fetch.rate':
            return new AppError(429, 'DOWNLOADER_RATE_LIMITED', 'The downloader is rate-limited right now. Please try again shortly.');
        ...
        default:
            return new AppError(502, 'AUDIO_EXTRACTION_FAILED', code ? `Audio extraction failed (${code}).` : `Audio extraction failed (cobalt ${status}).`);
    }
}
const igmp3Routes = async (app) => {
    app.get('/', {
        schema: {
            tags: ['download'],
        ...
        if (!mediaUrl) {
            ...
            throw new AppError(502, 'AUDIO_EXTRACTION_FAILED', 'The downloader did not return an audio file. Please try again.');
        }
...
//# sourceMappingURL=igmp3.routes.js.map