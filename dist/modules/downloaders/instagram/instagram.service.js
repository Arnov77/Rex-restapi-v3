...
function cobaltIsVideo(filename, sourceUrl) {
    const f = (filename || '').toLowerCase();
    if (/\.(mp4|mov|mkv|webm|m4v)$/.test(f))
        return true;
    if (/\.(jpe?g|png|webp|gif|heic)$/.test(f))
        return false;
    return /\/(reel|reels|tv)\//i.test(sourceUrl);
}
async function fetchViaCobalt(url, signal) {
    ...
    try {
        ...
        if (!res.ok)
            throw new Error(`cobalt returned ${res.status}`);
...
async function fetchViaEmbed(shortcode, signal) {
    const controller = new AbortController();
    ...
    try {
        ...
        if (!res.ok)
            throw new Error(`embed returned ${res.status}`);
        ...
        if (media.length === 0) {
            ...
            if (ogVideo)
                media.push({ type: 'video', url: ogVideo[1].replace(/&/g, '&') });
            const ogImage = html.match(/property="og:image"\s+content="([^"]+)"/);
            if (ogImage && !ogVideo)
                media.push({ type: 'image', url: ogImage[1].replace(/&amp;/g, '&') });
        }
...
async function fetchViaGraphQL(shortcode, signal) {
    const controller = new AbortController();
    const timeout = setTimeout(() => controller.abort(), 10_000);
    const mergedSignal = signal ? AbortSignal.any([signal, controller.signal]) : controller.signal;
    try {
        ...
        if (!item)
            throw new Error('no shortcode_media in response');
        const media = [];
...
export async function downloadInstagram(url, signal) {
