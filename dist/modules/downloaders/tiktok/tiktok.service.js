 ...
 */
async function fetchViaTikwm(url, signal) {
    ...
    if (d.images && d.images.length > 0) {
        return {
            ...
            media: [
                ...
                ...(d.music ? [{ type: 'audio', url: d.music_info?.play || d.music }] : []),
            ],
        };
    }
    // Video post
    ...
    const media = [];
    if (hdUrl)
        media.push({ type: 'video', url: hdUrl.startsWith('http') ? hdUrl : `https://www.tikwm.com${hdUrl}`, quality: 'hd' });
    if (sdUrl && sdUrl !== hdUrl)
        media.push({ type: 'video', url: sdUrl.startsWith('http') ? sdUrl : `https://www.tikwm.com${sdUrl}`, quality: 'sd' });
    if (d.music)
        media.push({ type: 'audio', url: d.music_info?.play || d.music, quality: undefined });
    return {
 ...
 */
export async function downloadTiktok(url, signal) {
