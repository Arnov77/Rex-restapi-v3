 ...
 */
async function fetchViaFxtwitter(tweetId, signal) {
    ...
    if (!json.tweet)
        throw new Error('fxtwitter: no tweet data');
 ...
 */
async function fetchViaVxtwitter(tweetId, signal) {
    const res = await fetch(`https://api.vxtwitter.com/Twitter/status/${tweetId}`, {
        headers: { 'User-Agent': 'Rex-API/3.0' },
        signal,
    });
    if (!res.ok)
        throw new Error(`vxtwitter returned ${res.status}`);
    const json = (await res.json());
    const media = [];
 ...
 */
export async function downloadTwitter(url, signal) {
