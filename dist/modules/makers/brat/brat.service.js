...
export async function generate(opts, { signal } = {}) {
    ...
    if (opts.bgImage)
        await assertPublicUrl(opts.bgImage);
...
async function generateStill(opts, signal) {
    ...
    const buffer = await withPage(async (page) => {
        ...
        const shotType = opts.format === 'jpeg' ? 'jpeg' : 'png';
        const shotOpts = { type: shotType };
        if (shotType === 'jpeg')
    ...
    if (!buffer || buffer.length === 0)
        throw Internal('Brat produced an empty buffer');
 ...
 */
async function captureFrames(opts, signal) {
    ...
    return withPage(async (page) => {
        ...
        for (let i = 0; i < frameCount; i++) {
            ...
            await page.evaluate((v) => {
                const el = globalThis.document.getElementById('t');
                if (el)
                    el.textContent = v;
            }, partial);
            const png = await page.screenshot({ type: 'png', omitBackground: false });
            if (!png || png.length === 0)
                throw Internal('Brat frame produced empty buffer');
            frames.push(png);
        }
        return frames;
    }, { viewport: { width: opts.width, height: opts.height }, signal });
}
async function generateAnimated(opts, signal) {
    ...
    if (!buffer || buffer.length === 0)
        throw Internal('Brat animation produced empty buffer');
    return { buffer, mimeType: MIME[opts.format], format: opts.format };
}
function encodeGif(opts, frames, holdMs) {
    ...
    for (let i = 0; i < frames.length; i++) {
        ...
        if (!sharedPalette)
            sharedPalette = quantize(rgba, 64, { format: 'rgb444' });
...
async function encodeWebp(opts, frames, holdMs) {
...
//# sourceMappingURL=brat.service.js.map