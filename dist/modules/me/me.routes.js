...
import { ConfirmPasswordBody, MeResponse, MyKeyResponse, RegenerateKeyResponse, RevealKeyResponse, UsageResponse, } from './me.schemas.js';
...
const meRoutes = async (app) => {
    ...
    app.post('/key/reveal', {
        ...
        schema: {
            ...
            description: 'Self-provisioned user keys are not stored encrypted by default. If reveal returns 404, regenerate to get a new plaintext.',
            security: [{ bearerAuth: [] }],
    ...
    app.post('/key/regenerate', {
        preHandler: [app.authenticate, ipLimit],
        schema: {
            hide: true,
            tags: ['me'],
            summary: "Rotate the secret of the user's API key (password required)",
            description: 'The same key id is reused, so today\'s usage counter persists — rotating the secret does not give a free quota reset.',
            security: [{ bearerAuth: [] }],
            body: ConfirmPasswordBody,
        ...
        return { ok: true, data: { plaintext: result.plaintext, key: view } };
    });
    app.get('/usage', {
        preHandler: [app.authenticate, ipLimit],
        schema: {
