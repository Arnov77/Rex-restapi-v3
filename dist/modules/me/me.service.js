...
export function meService(db) {
    ...
    async function resolveKey(userId) {
        ...
        if (!user.apiKeyId)
            throw NotFound('No API key linked to this user');
        const key = await keys.repo.findById(user.apiKeyId);
        if (!key)
            throw NotFound('API key no longer exists');
        return key;
    }
    /**
    ...
    async function confirmPassword(userId, password) {
        ...
        if (!ok)
            throw Forbidden('Password confirmation failed');
    }
    return {
        async getMe(userId) {
            const user = await users.findById(userId);
        ...
        async revealKey(userId, password) {
        ...
        async regenerateKey(userId, password) {
            await confirmPassword(userId, password);
            const key = await resolveKey(userId);
            if (key.revoked)
                throw Forbidden('Cannot regenerate a revoked key');
