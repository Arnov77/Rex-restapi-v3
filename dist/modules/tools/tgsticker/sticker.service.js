...
function getToken() {
    ...
    if (!t)
        throw Internal('TELEGRAM_BOT_TOKEN is not set');
    return t;
}
...
async function tgGet(path, signal) {
    const token = getToken();
    const res = await fetch(`${TG_API(token)}${path}`, { signal, dispatcher: ipv4Agent });
    const json = await res.json();
    if (!json.ok)
        throw BadRequest(`Telegram API error: ${json.description ?? 'unknown'}`);
    return json.result;
}
async function downloadTgFile(filePath, signal) {
    const token = getToken();
    const res = await fetch(`${TG_FILE(token)}/${filePath}`, { signal, dispatcher: ipv4Agent });
    if (!res.ok)
        throw Internal(`Failed to download Telegram file: ${res.statusText}`);
    return Buffer.from(await res.arrayBuffer());
}
async function getFileInfo(fileId, signal) {
    return tgGet(`/getFile?file_id=${encodeURIComponent(fileId)}`, signal);
}
async function getStickerSet(name, signal) {
    return tgGet(`/getStickerSet?name=${encodeURIComponent(name)}`, signal);
}
// ─── Image conversion ─────────────────────────────────────────────────────────
const MIME_MAP = {
    jpeg: 'image/jpeg',
...
async function convertSticker(buf, format, quality, size) {
    let s = sharp(buf);
    if (size)
        s = s.resize(size, size, { fit: 'contain', background: { r: 0, g: 0, b: 0, alpha: 0 } });
    if (format === 'jpeg')
        return s.flatten({ background: '#ffffff' }).jpeg({ quality }).toBuffer();
    if (format === 'webp')
        return s.webp({ quality, effort: 3 }).toBuffer();
    return s.png().toBuffer();
}
// ─── Cache ────────────────────────────────────────────────────────────────────
const CACHE_MAX = 100;
const CACHE_TTL_MS = 30 * 60 * 1000;
const singleCache = new LruCache({ max: CACHE_MAX, ttlMs: CACHE_TTL_MS });
const packCache = new LruCache({ max: 50, ttlMs: CACHE_TTL_MS });
...
// ─── Single sticker ───────────────────────────────────────────────────────────
export async function generateSingle(opts, { signal } = {}) {
    ...
    if (packName) {
        ...
        if (!first)
            throw BadRequest('Sticker pack is empty');
        fileId = first.file_id;
    }
    // Get file info
    const fileInfo = await getFileInfo(fileId, signal);
    if (!fileInfo.file_path)
        throw Internal('Telegram did not return file path');
    ...
    const result = { buffer, mimeType, format, isAnimated };
    singleCache.set(key, result);
    return result;
}
// ─── Pack ─────────────────────────────────────────────────────────────────────
export async function generatePack(opts, authorName, { signal } = {}) {
    const key = cacheKey({ ...opts, authorName });
    const cached = packCache.get(key);
    if (cached)
        return cached;
    const packName = extractPackName(opts.input);
    if (!packName)
        throw BadRequest('Pack input must be a t.me/addstickers/<name> link');
    const set = await getStickerSet(packName, signal);
...
export function lookupFile(filename) {
