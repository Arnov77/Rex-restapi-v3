...
const generateBratSchema = Joi.object({
  text: Joi.string().min(1).max(500).required().trim().messages({
  ...
  bgColor: Joi.string().pattern(COLOR_REGEX).optional().messages({
    'string.pattern.base': 'Invalid color format (hex or name)',
  }),

  textColor: Joi.string().pattern(COLOR_REGEX).optional(),
});
