...
const miqSchemas = {
  generateQuote: Joi.object({
    text: Joi.string().required().trim().min(5).max(500).messages({
    ...
    author: Joi.string().optional().trim().max(100).default('Unknown').messages({
    ...
    color: Joi.boolean().optional().default(false).messages({
