export interface IqcTemplateOptions {
  type: 'chat' | 'sticker' | 'media';
  text?: string;
  media?: string;
  time: string;
...
export function renderIqcHtml(opts: IqcTemplateOptions): string {
  const preset = TYPE_PRESETS[opts.type];
  const safeMessage = esc(opts.text ?? '');

  const mediaTime =
    opts.type === 'sticker' || opts.type === 'media'
      ? `<div class="media-time">${esc(opts.time)}</div>`
      : '';

  const sticker =
    opts.type === 'sticker' && opts.media
      ? `
