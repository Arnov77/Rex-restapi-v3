...
import type { QcQuery } from './qc.schemas.js';
...
export interface QcGenerateOptions {
  signal?: AbortSignal;
}
...
const cache    = new LruCache<string, QcResult>({ max: CACHE_MAX, ttlMs: CACHE_TTL_MS });
const inflight = new Map<string, Promise<QcResult>>();

function cacheKey(opts: QcQuery): string {
...
export async function generate(opts: QcQuery, { signal }: QcGenerateOptions = {}): Promise<QcResult> {
...
async function renderOnce(opts: QcQuery, signal?: AbortSignal): Promise<QcResult> {
  ...
  const png = await withPage(
    async (page) => {
      ...
      if (!el) throw Internal('QC canvas element not found');
