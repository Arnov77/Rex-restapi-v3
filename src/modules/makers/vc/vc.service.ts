...
import type { VcQuery } from './vc.schemas.js';
...
export interface VcGenerateOptions {
  signal?: AbortSignal;
}
...
export async function generate(opts: VcQuery, { signal }: VcGenerateOptions = {}): Promise<VcResult> {
  await assertPublicUrl(opts.audio);

  // Download audio ke buffer
  const res = await fetch(opts.audio, { signal });
  if (!res.ok) throw Internal(`Failed to fetch audio: ${res.statusText}`);
...
function runFfmpeg(args: string[], signal?: AbortSignal): Promise<void> {
  return new Promise((resolve, reject) => {
    ...
    proc.on('close', (code) => {
      ...
      if (code === 0) resolve();
      else reject(Internal(`FFmpeg exited with code ${code}: ${stderr.slice(-300)}`));
    });
