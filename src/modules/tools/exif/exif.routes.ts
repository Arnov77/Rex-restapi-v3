...
*/
const exifRoutes: FastifyPluginAsyncZod = async (app) => {
    ...
    async (req, reply) => {
      ...
      if (isMultipart) {
        if (typeof req.file !== 'function') {
          return reply.code(500).send({
            ...
            error: {
              message:
                '@fastify/multipart belum ter-register. Register plugin multipart dulu di app utama.',
            },
          });
        }
      ...
      if (!image) {
        return reply.code(400).send({
          ...
          error: {
            message:
              'Provide either ?image=<url> or upload a file as multipart/form-data field `file`',
          },
        });
      }
