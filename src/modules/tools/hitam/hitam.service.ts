...
async function hitamFromBase64(imgsJson: string, seed?: number): Promise<HitamResult> {
  ...
  for (let i = 0; i < attempts; i++) {
    ...
    if (!imageDataUri) throw new AppError(502, 'HITAM_NO_OUTPUT', 'HF Space returned no output', null, 'Could not process this image. Try again with a different one.');

    const match = /^data:(image\/[a-zA-Z0-9.+-]+);base64,(.+)$/.exec(imageDataUri);
    if (!match) throw new AppError(502, 'HITAM_BAD_OUTPUT_FORMAT', 'HF Space returned an unexpected image format', null, 'Could not process this image. Try again with a different one.');

    const mimeType = match[1];
    const base64Data = match[2];
    if (!mimeType || !base64Data) {
      throw new AppError(502, 'HITAM_BAD_OUTPUT_FORMAT', 'HF Space returned an unexpected image format', null, 'Could not process this image. Try again with a different one.');
    }
