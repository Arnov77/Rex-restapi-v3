...
import type { randomStickerQuery } from './randomsticker.schemas.js';
...
async function loadActivePacks(db: SupabaseClient): Promise<string[]> {
  ...
  const names = await randomStickerPacksRepo(db).listActiveNames();
...
export interface randomStickerResult {
  buffer: Buffer;
  mimeType: string;
  format: string;
  isAnimated: boolean;
...
export async function getRandomrandomSticker(
  db: SupabaseClient,
  opts: randomStickerQuery,
  signal?: AbortSignal,
): Promise<randomStickerResult> {
...
export const randomStickerService = { getRandomrandomSticker, invalidatePackListCache };
