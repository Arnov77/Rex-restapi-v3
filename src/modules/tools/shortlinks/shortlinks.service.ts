...
export function shortlinksService(db: SupabaseClient) {
  const repo = shortlinksRepo(db);

  return {
    async create(
      body: CreateShortlinkBody,
      opts: { userId?: string; apiKeyId?: string; isMaster?: boolean },
    ) {
    ...
    async resolve(slug: string) {
      ...
      if (!link) throw NotFound('Shortlink not found');
    ...
    async list(opts: { userId?: string; apiKeyId?: string }) {
      if (opts.userId) return repo.findByUserId(opts.userId);
      if (opts.apiKeyId) return repo.findByApiKeyId(opts.apiKeyId);
      return [];
    },

    async delete(slug: string, opts: { userId?: string; apiKeyId?: string }) {
      const link = await repo.findById(slug);
      if (!link) throw NotFound('Shortlink not found');

      // Pastikan hanya owner yang bisa hapus
      const isOwner =
        (opts.userId && link.user_id === opts.userId) ||
        (opts.apiKeyId && link.api_key_id === opts.apiKeyId);

      if (!isOwner) throw NotFound('Shortlink not found');
