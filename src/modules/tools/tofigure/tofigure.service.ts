...
import { TOFIGURE_STYLE, TOFIGURE_PROMPT } from './tofigure.schemas.js';

const HF_SPACE = 'prithivMLmods/Qwen-Image-Edit-2511-LoRAs-Fast';

export interface TofigureResult {
  buffer: Buffer;
  mimeType: string;
}
...
async function tofigureFromBase64(imgsJson: string, seed?: number): Promise<TofigureResult> {
  ...
  for (let i = 0; i < attempts; i++) {
    ...
    try {
      ...
      if (msg.includes('queue') || msg.includes('timeout')) {
        throw new AppError(503, 'TOFIGURE_QUEUE_TIMEOUT', 'HF Space queue timeout', null, 'Server sedang sibuk, coba lagi dalam beberapa saat.');
      }
      throw new AppError(502, 'TOFIGURE_INFERENCE_FAILED', `Gagal generate gambar: ${err?.message ?? 'unknown error'}`, null, 'Gagal memproses gambar. Coba dengan gambar lain.');
    }
    ...
    if (!outputUrl) throw new AppError(502, 'TOFIGURE_NO_OUTPUT', 'HF Space tidak menghasilkan gambar', null, 'Gagal memproses gambar. Coba lagi dengan gambar yang berbeda.');

    const dlRes = await fetch(outputUrl);
    if (!dlRes.ok) throw new AppError(502, 'TOFIGURE_DOWNLOAD_FAILED', `Gagal download hasil: ${dlRes.status}`, null, 'Gagal mengambil hasil gambar. Coba lagi.');
  ...
  throw new AppError(
    503,
    'TOFIGURE_QUOTA_EXHAUSTED',
    `Semua HuggingFace token kena quota ZeroGPU (${attempts} token)`, // → log internal
    null,
    'Fitur to figure sedang sibuk, coba lagi dalam beberapa menit.' // → response ke user
  );
}
 ...
 */
export async function tofigureFromUrl(imageUrl: string, seed?: number): Promise<TofigureResult> {
  await assertPublicUrl(imageUrl);
  
  const imgRes = await fetch(imageUrl);
  if (!imgRes.ok) throw new AppError(400, 'TOFIGURE_FETCH_FAILED', `Gagal fetch gambar: ${imgRes.status}`, null, 'URL gambar tidak bisa diakses. Pastikan URL valid dan publik.');
  
  const imgBuf = Buffer.from(await imgRes.arrayBuffer());
  const imgsJson = JSON.stringify([imgBuf.toString('base64')]);
  
  return tofigureFromBase64(imgsJson, seed);
}

/**
 * Handler jika input berupa Buffer (dari multipart form data / upload langsung)
 */
export async function tofigureFromBuffer(buffer: Buffer, seed?: number): Promise<TofigureResult> {
