...
async function tofigureFromBase64(imgsJson: string, seed?: number): Promise<TofigureResult> {
  ...
  for (let i = 0; i < attempts; i++) {
    ...
    try {
      ...
      if (msg.includes('queue') || msg.includes('timeout')) {
        throw new AppError(503, 'TOFIGURE_QUEUE_TIMEOUT', 'HF Space queue timeout', null, 'The server is busy right now. Please try again shortly.');
      }
      throw new AppError(502, 'TOFIGURE_INFERENCE_FAILED', `Image generation failed: ${err?.message ?? 'unknown error'}`, null, 'Could not process this image. Try a different one.');
    }
  ...
  throw new AppError(
    ...
    null,
    'The to figure feature is busy right now. Please try again in a few minutes.' // → response to user
  );
}
...
export async function tofigureFromUrl(imageUrl: string, seed?: number): Promise<TofigureResult> {
  ...
  if (!imgRes.ok) throw new AppError(400, 'TOFIGURE_FETCH_FAILED', `Failed to fetch image: ${imgRes.status}`, null, 'Could not access that image URL. Make sure it is valid and publicly accessible.');
