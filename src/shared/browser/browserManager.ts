...
function getPool(): PagePool {
  ...
  if (instance) pool.attach(instance);
  return pool;
}

// ── Public API ─────────────────────────────────────────────────────────

export interface WithPageOptions extends BrowserContextOptions {
  /**
   * AbortSignal from the incoming HTTP request. When fired:
   *   - If the caller is queued waiting for a slot → reject immediately,
   *     no Chromium work is done.
   *   - If the caller is mid-render → page.close() is called, which
   *     interrupts any in-flight navigation/screenshot and makes the
   *     awaited Playwright call throw. The page is then released back
