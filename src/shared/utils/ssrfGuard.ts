...
export async function assertPublicUrl(rawUrl: string, opts: AssertPublicUrlOpts = {}): Promise<URL> {
  ...
  if (net.isIP(hostname)) {
    if (isPrivateIP(hostname)) throw BadRequest('URL points to a private or reserved IP address');
    return url;
  }
  const resolver =
    opts.resolver ?? ((host: string) => dns.lookup(host, { all: true, verbatim: true }));
  ...
  if (!answers.length) throw BadRequest(`DNS lookup returned no answers for "${hostname}"`);
